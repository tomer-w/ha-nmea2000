...
class NMEA2000ConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_options(self, user_input=None):
        _LOGGER.debug("async_step_options called with user_input: %s, data: %s", user_input, self.data)
        errors = {}
        ...
        return self.async_show_form(
            step_id="options",
            data_schema=USB_DATA_SCHEMA if self.data["mode"] == "USB" else TCP_DATA_SCHEMA,
            errors=errors,
        )
...
class OptionsFlowHandler(config_entries.OptionsFlow):
    async def async_step_init(self, user_input=None):
        ...
        if user_input is not None:
            ...
            return self.async_show_form(
                step_id="init",
                data_schema=self.add_suggested_values_to_schema(USB_DATA_SCHEMA if current_data["mode"] == "USB" else TCP_DATA_SCHEMA, current_data),
            )