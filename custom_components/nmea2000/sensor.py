...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry, async_add_entities: AddEntitiesCallback) -> None:
    ...
    _LOGGER.info(f"Configuring sensor with name: {name}, mode: {mode}, PGN Include: {pgn_include}, PGN Exclude: {pgn_exclude}")
    ...
    if (mode == "USB"):
        ...
        _LOGGER.info(f"USB sensor with name: {name}, serial_port: {serial_port}, baudrate: {baudrate}")
        sensor = SerialSensor(
            name,
            serial_port,
            baudrate,
