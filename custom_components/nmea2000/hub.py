...
class Hub:
    ...
    def __init__(self, hass: HomeAssistant, entry: ConfigEntry) -> None:
        ...
        self.time_between_updates = timedelta(
            milliseconds=entry.data.get(CONF_MS_BETWEEN_UPDATES, 5000)
        )
        ...
        if gateway_type == GatewayType.WAVESHARE:
            ...
            self.gateway = ActisenseBstNmea2000Gateway(
                host=ip, port=port, **common_kwargs
            )
            ...
            _LOGGER.info(
                "python-can with interface: %s, channel: %s, bitrate: %s",
                can_interface,
                can_channel,
                can_bitrate,
            )
        ...
        while not await event_wait(self.stop_event, message_rate_interval):
            ...
            if elapsed_time > 0:
                ...
                _LOGGER.debug(
                    ...
                    self.message_count_per_interval,
                    elapsed_time,
    ...
    async def receive_callback(self, message: NMEA2000Message) -> None:
        ...
        device_name = (
            f"{message.description} ({message.source_iso_name.manufacturer_code} - {message.source_iso_name.device_function} - {message.source_iso_name.unique_number}) (PK: {message.hash})"
            if message.source_iso_name is not None
            else f"{message.description} (PK: {message.hash})"
        )

        # Process individual fields in the message
        for field in message.fields:
            # Expand repeating field lists into individual sensors
            if field.type == FieldTypes.VARIABLE and isinstance(field.value, list):
                for entry_idx, entry in enumerate(field.value):
                    assert isinstance(entry, dict), (
                        f"Expected list of dicts for variable field, got {type(entry)}"
                    )
                    for sub_field_id, sub_field in entry.items():
                        assert isinstance(sub_field, NMEA2000Field), (
                            f"Expected NMEA2000Field, got {type(sub_field)}"
                        )
                        # Skip reserved/spare fields
            ...
            if field.id == "sid":
                _LOGGER.debug(
                    "Skipping field ids. Name: %s and type: %s", field.name, field.type
                )
                continue
